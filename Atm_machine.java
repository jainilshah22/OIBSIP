...
class bankaccount{
    static  void register(){
        ...
        Atm_machine .name=sc.nextLine();
        System.out.println("Enter username :");
        String user=sc.nextLine();
        System.out.println("Enter password :");
        String pass=sc.nextLine();
        System.out.println("Enter your Account number :");
        Atm_machine .accnumber=sc.nextLine();
        System.out.println("REGISTRATION SUCCESSFULLY!");
        System.out.println("---------------------------");
        Atm_machine .prompt();
        while(true){
            display(Atm_machine .name);
...
class transaction{
    static void withdraw(){
        ...
        if(wcash<=Atm_machine .balance){
            Atm_machine .balance=Atm_machine .balance-wcash;
            Atm_machine .history.add(Integer.toString(wcash));
            Atm_machine .history.add("Withdraw");
    ...
    static void deposit(){
        ...
        Atm_machine .updatebalance(dcash);
        Atm_machine .history.add(Integer.toString(dcash));
        Atm_machine .history.add("Deposit");
    ...
    static void transfer(){
        ...
        if(tcash<=Atm_machine .balance){
            Atm_machine .balance=Atm_machine .balance-tcash;
            Atm_machine .history.add(Integer.toString(tcash));
            Atm_machine .history.add("transferred");
...
class his{
    static void transactionhistory(){
            ...
            int k=0;
            if(Atm_machine .balance>0){
            for(int i=0;i<(Atm_machine .history.size()/2);i++)
            {
                for(int j=0;j<2;j++)
                {
                    System.out.print(Atm_machine .history.get(k)+" ");
                    k++;
                }
                System.out.println("---------------------");
            }
...
public class Atm_machine {
    ...
    static void prompt(){
        Scanner sc=new Scanner(System.in);
        System.out.println("WELCOME "+Atm_machine .name+"! TO ATM SYSTEM");
        System.out.println("---------------------");
        System.out.println("Select option : ");
